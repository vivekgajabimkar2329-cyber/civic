...
export const ReportIssue = () => {
  ...
  const [formData, setFormData] = useState(() => {
    const savedDraft = localStorage.getItem('civicAiReportDraftPublic');
    if (savedDraft) {
      try {
  ...
  const handleSaveDraft = () => {
    ...
    localStorage.setItem('civicAiReportDraftPublic', JSON.stringify(draftState));
  ...
  const handleReset = () => {
    if (window.confirm('Are you sure you want to reset the form?')) {
      ...
      localStorage.removeItem('civicAiReportDraftPublic');
      setLastSaved(null);
  ...
  // Header display details matching mockup
  const displayName = user?.name || 'Admin User';
  const displayLocation = 'Hyderabad';
  const displayAvatar = 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=facearea&facepad=2&w=256&h=256&q=80';
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col">
      ...
      <header className="bg-white border-b border-slate-200/80 sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-teal-800 text-white flex items-center justify-center font-black text-lg tracking-wider shadow shadow-teal-800/10">
            C
          </div>
          <div>
            <div className="text-base font-black text-slate-900 tracking-tight flex items-center gap-1">
              CIVIC AI
            </div>
        ...
        <div className="flex items-center gap-5">
          <button type="button" className="relative p-2 text-slate-450 hover:text-slate-700 bg-slate-50 rounded-full border border-slate-200 transition-colors focus:outline-none">
            <Bell size={18} />
            <span className="absolute top-1 right-1 w-4 h-4 bg-rose-600 text-white text-[9px] font-black rounded-full flex items-center justify-center shadow-sm">
              2
            </span>
          </button>
          ...
          <div className="flex items-center gap-3 border-l border-slate-200 pl-5">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-extrabold text-slate-900 leading-tight">{displayName}</p>
              <p className="text-[10px] font-semibold text-slate-450 uppercase mt-0.5">{displayLocation}</p>
            </div>
            <div className="relative group">
              <img
                src={displayAvatar}
                alt="Profile"
                className="w-10 h-10 rounded-full object-cover border border-slate-200 shadow-sm cursor-pointer hover:border-teal-800 transition-colors"
              />
              <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-xl py-2 hidden group-hover:block animate-in fade-in duration-100">
                <Link to="/login" className="block px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Portal Login</Link>
                <Link to="/" className="block px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 border-t border-slate-100">Home Page</Link>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Main content body wrapper with background layout */}
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-8 animate-in fade-in duration-300">
        
        {/* Toast Notification */}
        {toast && (
          <div className={`fixed top-20 right-4 z-50 flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold text-white shadow-lg ${
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex items-center gap-2 bg-teal-50/50 border border-teal-100/80 rounded-xl px-4 py-3">
              <div className="p-1.5 bg-teal-100/50 rounded-lg text-teal-705">
                <Brain size={18} />
              </div>
              <div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      { val: 'Anonymous', icon: Lock, color: 'border-slate-300 bg-slate-50 text-slate-550' }
                    ].map((option) => {
                  ...
                  {showTagInput ? (
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        placeholder="Tag..."
                        className="px-2 py-1 text-xs border rounded-lg outline-none focus:border-teal-805 w-24 bg-white"
                        autoFocus
                      />
                      <button
                        type="button"
            ...
            <div ref={locationRef} id="location" className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <button
                    type="button"
                    className="flex items-center gap-3 p-4 border border-slate-205 bg-slate-50/30 hover:bg-slate-50 rounded-2xl text-left focus:outline-none transition-all"
                  >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button
                    type="button"
                    onClick={triggerLocationFetch}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-teal-705 hover:text-teal-900 p-1.5 hover:bg-teal-50 rounded-lg"
              ...
              {formData.images.length > 0 && (
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                  ...
                  <label className="relative aspect-square border border-dashed border-slate-350 hover:border-slate-400 rounded-xl flex flex-col items-center justify-center cursor-pointer text-slate-500 hover:text-slate-700 bg-slate-50/50 hover:bg-slate-50 transition-colors">
                    ...
                    />
                    <Plus size={20} className="mb-0.5 text-slate-450" />
              ...
              <div className="p-4 space-y-4">
                ...
                <div>
                  ...
                  <button
                    type="button"
                    onClick={triggerLocationFetch}
                    className="inline-flex items-center gap-1 text-xs font-bold text-teal-850 hover:text-teal-900 mt-2 hover:underline focus:outline-none"
                  >
                    <Compass size={12} />
            ...
            <div className="bg-blue-50/30 border border-blue-100 rounded-2xl p-5 shadow-sm space-y-4">
              ...
              <ul className="space-y-3">
                {[
                  ...
                  return (
                    <li key={idx} className="flex items-center gap-2.5 text-xs font-semibold text-slate-755">
        ...
        <div className="mt-8 bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={handleReset}
              className="p-2.5 text-slate-400 hover:text-slate-600 bg-slate-50 border border-slate-205 rounded-xl hover:bg-slate-100 transition-all focus:outline-none"
              title="Reset Form"
            >
              <RotateCcw size={16} />
            </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 p-8 text-center space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-2">
              ...
              <button
                type="button"
                onClick={() => navigate(user ? '/my-complaints' : '/track')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-5 border-b border-slate-100">
              ...
              <button onClick={() => setShowPreviewModal(false)} className="text-slate-400 hover:bg-slate-55 p-1.5 rounded-lg transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              ...
              <div className="bg-slate-50 p-4 rounded-xl">
                ...
                <p className="text-xs font-medium text-slate-705 leading-relaxed mt-1">{formData.description}</p>
              </div>
