...
class ComplaintService:
    ...
    def confirm_resolution(self, complaint_id: str, user, confirmed: bool, comment: str = None) -> Complaint:
        complaint = self.get_complaint_by_id(complaint_id)
        if complaint.user != user:
            raise PermissionDenied("You do not have permission to modify this complaint.")
        
        status_value = "CLOSED" if confirmed else "IN_PROGRESS"
        return self.repository.update(complaint, {"status": status_value})
